...
const App: React.FC = () => {
  ...
  const handleAgentInteraction = async (customPrompt?: string, forcedRole?: AgentRole) => {
    ...
    const initialSteps: AgentStep[] = agentMode === AgentMode.MULTI ? [
      { id: '1', label: 'วิเคราะห์โครงสร้าง (Analysis)', status: 'active', timestamp: Date.now(), role: AgentRole.ARCHITECT },
      { id: '2', label: 'สังเคราะห์ส่วนหน้า (Frontend)', status: 'pending', timestamp: Date.now(), role: AgentRole.FRONTEND },
      { id: '3', label: 'ตรวจสอบระบบ (Validation)', status: 'pending', timestamp: Date.now(), role: AgentRole.DEVOPS }
    ] : [
      { id: '1', label: `ประมวลผลงาน: ${role}`, status: 'active', timestamp: Date.now(), role }
    ];
